function [result] = THG2024scan1D(A,B,option,detection,farfield,geometry,nom,chi)

% THG2016 - Computes THG signal as a function of user defined focvusin
% parameters and geometry
%
% FILE NAME: THG2024scan1D.m
% AUTHOR: N Olivier (nicolas.olivier@polytechnique.edu)
% CREATED: 2006
% UPDATED: 2024/07/13
% VERSION: almost_Final
%
...
%for cx=-geometry:geometry
    for ct=30:60
        ...
        param=[cx,cy];
        [C,lim]=geometryRBC(longueur,largeur,b,bz,option,param,chi);
        A=[A lim]; %Save parameter for this geometry
        geotmp=squeeze(C(:,:,longueur));
        geotmp=abs(geotmp/max(max(geotmp)));
        ...
        for f=1:2*ff_step+1
            for g=1:2*ff_step+1
                ...
                T = phasefast(x,y,z,largeur,longueur,b,bz,lambda_400,n1_400);
